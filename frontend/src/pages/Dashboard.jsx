    ...
    <div className="p-6 space-y-6 font-sans h-[calc(100vh-4rem)] overflow-y-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        <Card hoverEffect className="relative overflow-hidden border-sky-500/10">
          ...
          <h3 className="text-3xl font-extrabold text-slate-100 light:text-slate-800 mt-2 font-sans tracking-tight">
            {totalInquiries}
          </h3>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-300 light:text-slate-700 tracking-wider uppercase">
              Recent Email Inquiries
            </h3>
            <Link to="/kanban" className="text-xs font-semibold text-sky-400 hover:text-sky-300 flex items-center gap-1 group">
              ...
              <div className="divide-y divide-white/5 light:divide-slate-200/50">
                {recentInquiries.map((task) => (
                  <Link 
                    key={task.id} 
                    to={`/inquiry/${task.id}`} 
                    className="flex items-center justify-between p-5 hover:bg-slate-800/30 light:hover:bg-slate-50/50 transition-all duration-200 group"
                  >
                    <div className="space-y-1.5 max-w-[70%]">
                      <div className="flex items-center gap-2.5">
                        <span className="text-xs font-bold text-sky-400 tracking-wide font-sans">
                          {task.inquiryId}
                        </span>
                        <h4 className="text-sm font-semibold text-slate-100 light:text-slate-800 group-hover:text-sky-400 transition-colors truncate">
                          {task.subject}
                        </h4>
                      </div>
                      <p className="text-xs text-slate-400 truncate">
                        Customer: <span className="font-semibold text-slate-300">{task.customerName}</span> &bull; Sender: <span className="text-slate-400">{task.senderEmail}</span>
                      </p>
                    </div>
        ...
        <div className="space-y-4">
          ...
          <Card className="space-y-6">
              ...
              {statuses.map((status) => {
                ...
                return (
                  <div key={status} className="space-y-1.5">
                    ...
                    {/* SVG progress line */}
                    <div className="w-full h-2 rounded-full bg-slate-900/50 light:bg-slate-200 overflow-hidden">
                      <div 
            ...
            {/* Quick Summary Box */}
            <div className="pt-5 border-t border-white/5 light:border-slate-200/50 text-xs text-slate-400 flex items-center justify-between">
